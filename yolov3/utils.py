...
CLASS_LIST = create_class_list('yolov3/data/coco.names')
...
def add_boxes_to_image(image, boxes, class_list, model_res):
    ...
    for b in boxes:
        ...
        if class_list[cls] == 'person':
            ...
            cv2.rectangle(image, start, (start[0] + t_w, start[1] + t_h + 3), color=COLORS[cls % c], thickness=-1)
            cv2.putText(image, class_list[cls], (start[0], start[1]+t_h-3), font, 0.5, color=text_color, thickness=1)
    return image


def detect(model, image, device, obj_threshold=0.7, nms_thresh=0.4, model_res=416):
    model.eval()
...
def boxes_array(image, boxes, class_list, model_res):
    h = image.shape[0]/model_res
    w = image.shape[1]/model_res
    c = len(COLORS)
    font = cv2.FONT_HERSHEY_SIMPLEX
    ...
    for b in boxes:
        ...
        if class_list[cls] == 'person':
            ...
            array = [int(b[0]*w), int(b[1]*h), int(b[2]*w), int(b[3]*h)]
            output.append(array)
    return output

def get_bbox(model, image, device, obj_threshold=0.7, nms_thresh=0.4, model_res=416):
    model.eval()
    resized = cv2.resize(image, (model_res, model_res))
    tfms = transforms.ToTensor()
    x = tfms(resized).unsqueeze(0).to(device)
    ...
    return boxes_array(image, boxes, CLASS_LIST, model_res)
